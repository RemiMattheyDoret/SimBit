...
void T4TreeRec::simplify(Pop& pop)
{
	...
	// Map to offsprings
	std::map<size_t, std::vector<size_t>> mapToOffsprings;
	...
	// Loop through nodes
	assert(Ni.size());
	int Ei_index_forMap = Ei.size()-1; // int so that it goes below zero and we know that we don't need to search anymore
	for (size_t u = Ni.size() - 1 ; u < Ni.size() ; --u)
	{
		// Select parental edges
		auto mapIt = mapToOffsprings.find(u); // O(log(n))
		if (mapIt != mapToOffsprings.end())
		{
			// node u has offsprings
			auto& parentEdgeIndices = *mapIt;
			...
			// update map - erase
			mapToOffsprings.erase(mapIt);
		}


		// update map - add u to parent of u
		while (Ei_index_forMap >= 0 && Ei[Ei_index_forMap].child != u)...

		// Set v (aka newID) to -1
		int v = -1;
